...
numFolders = 2; % Number of folders to get images from
mainDir = "3.3_glasses"; % Name of main directory
...
for i=1:size(folders,1)
    ...
    for j = 1:num1
        if((~(isempty(strfind(files(j).name,types(io)))) && ~(isempty(strfind(files(j).name,"glasses")))))
            img = imread(files(j).name);
            img = rgb2gray(img);
